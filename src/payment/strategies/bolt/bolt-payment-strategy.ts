...
export default class BoltPaymentStrategy implements PaymentStrategy {
    ...
    async initialize(options: PaymentInitializeOptions): Promise<InternalCheckoutSelectors> {
        ...
        this._useBoltClient = !!(bolt && bolt.useClientScript);
    ...
    async execute(payload: OrderRequestBody, options?: PaymentRequestOptions): Promise<InternalCheckoutSelectors> {
        if (this._useBoltClient) {
            ...
            return this._executeFullCheckoutTakeOver(payload, options);
        }
    }
    ...
    private async _executeWithBoltClient(payload: OrderRequestBody, options?: PaymentRequestOptions): Promise<InternalCheckoutSelectors> {
        ...
        const state = await this._store.dispatch(this._paymentMethodActionCreator.loadPaymentMethod(this._methodId, options));
        const paymentMethod = state.paymentMethods.getPaymentMethod(this._methodId);
    ...
    private async _executeFullCheckoutTakeOver(payload: OrderRequestBody, _options?: PaymentRequestOptions): Promise<InternalCheckoutSelectors> {
        const { payment, ...order } = payload;
