...
describe('BarclaycardPaymentStrategy', () => {
    ...
    it('initializes offsite payment flow', async () => {
        ...
        expect(paymentActionCreator.initializeOffsitePayment).toHaveBeenCalledWith(options.methodId, options.gatewayId, expect.objectContaining(expectedPayload));
    ...
    it('returns checkout state when initializing', async () => {
        const output = await strategy.initialize();

        expect(output).toEqual(store.getState());
    });

    it('returns checkout state when deinitializing', async () => {
        const output = await strategy.deinitialize();
